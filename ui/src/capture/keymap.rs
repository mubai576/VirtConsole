...
pub fn code_to_keycode(code: &str) -> Option<u32> {
    ...
    let v = match code {
        ...
        "ControlRight" => 0x9d,
        "ShiftLeft" => 0x2a,
        "ShiftRight" => 0x36, // 右 Shift 不是扩展键，就是 0x36
        "AltLeft" => 0x38,
        "AltRight" => 0xb8,
        // 方向键：**扩展键**，必须带 0x80。
...
#[allow(dead_code)]
pub const ALIASES: &[(&str, &str)] = &[("OSLeft", "MetaLeft"), ("OSRight", "MetaRight")];
...
pub fn char_to_shift_keycode(c: char) -> Option<(u32, bool)> {
    let (kc, shift) = match c {
        'a'..='z' => (
            code_to_keycode(&format!("Key{}", c.to_ascii_uppercase()))?,
            false,
        ),
...
mod tests {
    ...
    fn webkit_legacy_aliases_map_like_standard_names() {
        for (alias, canonical) in ALIASES {
            ...
            assert!(
                a.is_some(),
                "别名缺映射: {alias}（WebKitGTK 实际发的就是这个名字）"
            );
    ...
    fn char_map_agrees_with_code_map_for_letters() {
        for c in 'a'..='z' {
            ...
            assert_eq!(
                char_to_shift_keycode(c),
                Some((via_code, false)),
                "小写 {c} 不一致"
            );
            assert_eq!(
                char_to_shift_keycode(up),
                Some((via_code, true)),
                "大写 {up} 不一致"
            );
        }
