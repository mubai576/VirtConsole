...
export const suiteG = {
  ...
  async run() {
    ...
    // V3.0：缩放不再用 documentElement.style.zoom 的固定表，改为 rem × --vc-user-scale
    // （root font-size = 0.625vw × scale，物理尺寸随屏幕自动一致）。
    // 选项顺序 auto / compact / normal / large / xlarge，→ 一次落到 compact = 0.9。
    await step("G4_scale_apply_user_scale", async () => {
      ...
      const v = document.documentElement.style.getPropertyValue("--vc-user-scale").trim();
      assert(v === "0.9", `--vc-user-scale=${v}`);
      const px = parseFloat(getComputedStyle(document.documentElement).fontSize);
      assert(px > 4 && px < 64, `root font-size 异常 ${px}px`);
      await invoke("set_ui_scale", { scale: "auto" });
      document.documentElement.style.setProperty("--vc-user-scale", "1");
      await goHome();
    });
