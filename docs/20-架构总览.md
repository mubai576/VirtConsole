...
开机
 └─ virtconsole-weston.service     Weston（DRM 后端 + kiosk-shell）
     ...
     └─ virtconsole.service        vc-ui（Tauri 2 应用）
         ExecStartPre 等 Wayland socket 就绪后才启动
```
...
```text
模式 1：VM ──QMP screendump──→ qmp.rs ──PPM 解析──→ RGB
模式 2：VM ──D-Bus Scanout/Update──→ capture/frame.rs ──→ RGB
                                                          │
                                        base64 ──→ vm-frame 事件 ──→ 前端 Canvas
```

两条路径**数据形态一致**（RGB → Canvas），前端渲染层不区分来源。这是 V2.0 选像素事件
路径的主要原因，见 [21](21-采集与输入.md)。
