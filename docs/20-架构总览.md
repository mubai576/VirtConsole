...
| `VIRTCONSOLE_DMABUF` | 未设置 | 旧 EGL 导入实验；保持关闭 |
| `VIRTCONSOLE_SCANOUT_MAP` | 未设置 | Map 探针/overlay；保持关闭 |

两个实验开关都不是公共 IPC 或配置结构；删除对应 override 即可回到默认像素链路。
...
| `VIRTCONSOLE_DMABUF=1` | 失败的 EGL DMABUF 实验，默认不启用 |
| `VIRTCONSOLE_SCANOUT_MAP=1` | ScanoutMap 探针/实验模式，默认不启用 |
